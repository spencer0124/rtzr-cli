...
9. **실제 Claude(claude.ai)에 MCP를 연결해 큰 오디오(50초 mp3)를 전사하려다 실패**
   ...
   **미리** 못박아야 모델이 애초에 무리한 시도를 안 한다. 그리고 크기 초과는 조용히 실패하지 말고 즉시
   ...
   해봐" 같은 부분 수정에 만족하지 말고 왜 필요한지 원리를 재검토할 것. `internal-docs/concept.md` §8.3 참고.

10. **"core가 단일 소스"라는 원칙이 CLI/MCP 양쪽에서 조용히 깨져 있었음**
    사용자가 MCP로 단어별 타임스탬프(`use_word_timestamp`)를 요청했다가 그 옵션 자체가 tool에 없어서
    실패. RTZR 공식 문서(`developers.rtzr.ai/docs/stt-file/`)와 core/CLI/MCP를 교차검증해보니 두 가지
    문제가 겹쳐 있었음:
    - **진짜 버그(core)**: API가 `use_word_timestamp: true`면 `utterances[].words[]`를 내려주는데,
      `RawUtterance`/`Utterance`/`parseTranscript`가 이 필드를 전혀 파싱 안 함(`--json`은 raw를 그대로
      dump하니 우연히 안 깨졌지만 구조화된 타입으로는 접근 불가).
    - **파라미터 누락(MCP)**: `wordTimestamps`, `itn`, `disfluencyFilter`, `profanityFilter`, `domain`,
      `languageCandidates`, `paragraphSplitter`/`Max` 전부 MCP tool에 없었음. `languageCandidates`/
      `paragraphSplitter`는 CLI에도 없었음(core에는 있었는데). **`paragraph_splitter`는 API 기본값이
      `true`**라 아무도 노출 안 했어도 조용히 적용되고 있었음 — 사용자가 제어할 방법이 없었던 것.
    → 교훈: "core가 단일 소스, CLI/MCP는 파생"이라는 원칙은 스키마에 필드를 추가하는 것과 각 표면의
    플래그/tool 파라미터에 실제로 노출하는 것 사이에 **수동 동기화 단계**가 남아있는 한 계속 깨질 수
    있다. `baseTranscribeConfigSchema.shape`의 키 목록을 기준으로, CLI의 `toTranscribeConfig`와 MCP의
    `buildTranscribeConfig`가 모든 필드를 커버하는지 확인하는 회귀 테스트를 양쪽에 추가해서 재발
    방지(`packages/cli/src/config-mapping.test.ts`, `packages/mcp-worker/src/handler.test.ts`의
    "covers every field" 테스트). 이번 김에 CLI 패키지의 **첫 유닛테스트**도 여기서 생김(전엔 0개였음).
